# Get stats from climatic indexes including growing season calculations
# H. Achicanoy
# CIAT, 2016
...
wrapClimInd <- lapply(1:nrow(countyList), function(i){
  ...
  # Load growing seasons estimations
  load(paste('/mnt/workspace_cluster_8/Kenya_KACCAL/results/climatic_indices/historical/growing_seasons/', gsub(pattern=' ', replacement='_', countyList$County[[i]]),'_growingSeasons.RData', sep=''))
  db <- clim_indexes2[[1]]
  db <- db[,c('cellID', 'gSeason', paste(1981:2005))]
  # GPSV: Stability in start of season
  gpsv <- rbind(data.frame(Average=as.numeric(apply(X = db[db$gSeason==1, 3:ncol(db)], MARGIN = 2, FUN = sd, na.rm=TRUE)), Index='GPSV', Years=1981:2005, Season='First'),
                data.frame(Average=as.numeric(apply(X = db[db$gSeason==2, 3:ncol(db)], MARGIN = 2, FUN = sd, na.rm=TRUE)), Index='GPSV', Years=1981:2005, Season='Second'))
  # LGPM: Length of growing season
  db <- clim_indexes2[[2]]
  db <- db[,c('cellID', 'gSeason', paste(1981:2005))]
  lgpm <- rbind(data.frame(Average=as.numeric(apply(X = db[db$gSeason==1, 3:ncol(db)], MARGIN = 2, FUN = median, na.rm=TRUE)), Index='LGPM', Years=1981:2005, Season='First'),
                data.frame(Average=as.numeric(apply(X = db[db$gSeason==2, 3:ncol(db)], MARGIN = 2, FUN = median, na.rm=TRUE)), Index='LGPM', Years=1981:2005, Season='Second'))
  # LGPV: Variability of LGP
  lgpv <- rbind(data.frame(Average=as.numeric(apply(X = db[db$gSeason==1, 3:ncol(db)], MARGIN = 2, FUN = sd, na.rm=TRUE)), Index='LGPV', Years=1981:2005, Season='First'),
                data.frame(Average=as.numeric(apply(X = db[db$gSeason==2, 3:ncol(db)], MARGIN = 2, FUN = sd, na.rm=TRUE)), Index='LGPV', Years=1981:2005, Season='Second'))
  
  allWrap <- rbind(allWrap, gpsv, lgpm, lgpv)
  
  rm(first_seasonWrap, second_seasonWrap, clim_indexes2, db, gpsv, lgpm, lgpv)
  ...
  if(!file.exists(paste(outDir, '/', gsub(pattern=' ', replacement='_', countyList$County[[i]]), '_historicalTrend_LGP.pdf', sep=''))){
    ggsave(filename=paste(outDir, '/', gsub(pattern=' ', replacement='_', countyList$County[[i]]), '_historicalTrend_LGP.pdf', sep=''), plot=gg, width=9, height=8, units='in')
  }
  ...
  if(!file.exists(paste(outDir, '/', gsub(pattern=' ', replacement='_', countyList$County[[i]]), '_significanceTrend_LGP.pdf', sep=''))){
    ggsave(filename=paste(outDir, '/', gsub(pattern=' ', replacement='_', countyList$County[[i]]), '_significanceTrend_LGP.pdf', sep=''), plot=gg, width=9, height=8, units='in')
  }
...
wrapClimInd_2015 <- lapply(1:nrow(countyList), function(i){
  ...
  gg <- gg + theme(axis.text.x = element_text(size=7))
  outDir <- paste('/mnt/workspace_cluster_8/Kenya_KACCAL/results/graphics/historical_trends/', gsub(pattern=' ', replacement='_', countyList$County[[i]]), sep='')
  if(!file.exists(paste(outDir, '/', gsub(pattern=' ', replacement='_', countyList$County[[i]]), '_historicalTrend_2015_LGP.pdf', sep=''))){
    ggsave(filename=paste(outDir, '/', gsub(pattern=' ', replacement='_', countyList$County[[i]]), '_historicalTrend_2015_LGP.pdf', sep=''), plot=gg, width=9, height=8, units='in')
  }
  ...
  gg <- gg + geom_hline(yintercept=0.05)
  gg <- gg + ylab('p-value Mann-Kendall test') + scale_y_continuous(limits=c(0, 1))
  if(!file.exists(paste(outDir, '/', gsub(pattern=' ', replacement='_', countyList$County[[i]]), '_significanceTrend_2015_LGP.pdf', sep=''))){
    ggsave(filename=paste(outDir, '/', gsub(pattern=' ', replacement='_', countyList$County[[i]]), '_significanceTrend_2015_LGP.pdf', sep=''), plot=gg, width=9, height=8, units='in')
  }
...
wrapFutClimInd <- lapply(1:nrow(countyList), function(g){
  ...
  ###
  wrapFutClimInd_LGP <- lapply(1:length(periodList), function(i){
    
    cat('LGP-Period:', periodList[i], '\n')
    
    wrapClimInd <- lapply(1:length(rcpList), function(j){
      
      cat('LGP-RCP:', rcpList[j], '\n')
      
      wrapClimInd <- lapply(1:length(gcmList), function(k){
        
        cat('LGP-GCM:', gcmList[k], '\n')
        
        years_analysis <- unlist(strsplit(periodList[i], split='_'))
        years_analysis <- as.numeric(years_analysis)
        
        # Load growing seasons estimations
        load(paste('/mnt/workspace_cluster_8/Kenya_KACCAL/results/climatic_indices/future/growing_seasons/', gcmList[k], '/', periodList[i], '/', rcpList[j], '/', gsub(pattern=' ', replacement='_', countyList$County[[g]]), '_growingSeasons.RData', sep=''))
        db <- clim_indexes2[[1]]
        db <- db[,c('cellID', 'gSeason', paste(years_analysis[1]:years_analysis[2]))]
        # GPSV: Stability in start of season
        gpsv <- rbind(data.frame(Average=as.numeric(apply(X = db[db$gSeason==1, 3:ncol(db)], MARGIN = 2, FUN = sd, na.rm=TRUE)), Index='GPSV', GCM=gcmList[k], RCP=rcpList[j], Period=periodList[i], County=countyList$County[g], Season='first', Years=years_analysis[1]:years_analysis[2]),
                      data.frame(Average=as.numeric(apply(X = db[db$gSeason==2, 3:ncol(db)], MARGIN = 2, FUN = sd, na.rm=TRUE)), Index='GPSV', GCM=gcmList[k], RCP=rcpList[j], Period=periodList[i], County=countyList$County[g], Season='second', Years=years_analysis[1]:years_analysis[2]))
        # LGPM: Length of growing season
        db <- clim_indexes2[[2]]
        db <- db[,c('cellID', 'gSeason', paste(years_analysis[1]:years_analysis[2]))]
        lgpm <- rbind(data.frame(Average=as.numeric(apply(X = db[db$gSeason==1, 3:ncol(db)], MARGIN = 2, FUN = median, na.rm=TRUE)), Index='LGPM', GCM=gcmList[k], RCP=rcpList[j], Period=periodList[i], County=countyList$County[g], Season='first', Years=years_analysis[1]:years_analysis[2]),
                      data.frame(Average=as.numeric(apply(X = db[db$gSeason==2, 3:ncol(db)], MARGIN = 2, FUN = median, na.rm=TRUE)), Index='LGPM', GCM=gcmList[k], RCP=rcpList[j], Period=periodList[i], County=countyList$County[g], Season='second', Years=years_analysis[1]:years_analysis[2]))
        # LGPV: Variability of LGP
        lgpv <- rbind(data.frame(Average=as.numeric(apply(X = db[db$gSeason==1, 3:ncol(db)], MARGIN = 2, FUN = sd, na.rm=TRUE)), Index='LGPV', GCM=gcmList[k], RCP=rcpList[j], Period=periodList[i], County=countyList$County[g], Season='first', Years=years_analysis[1]:years_analysis[2]),
                      data.frame(Average=as.numeric(apply(X = db[db$gSeason==2, 3:ncol(db)], MARGIN = 2, FUN = sd, na.rm=TRUE)), Index='LGPV', GCM=gcmList[k], RCP=rcpList[j], Period=periodList[i], County=countyList$County[g], Season='second', Years=years_analysis[1]:years_analysis[2]))
        
        wrapClimInd <- rbind(gpsv, lgpm, lgpv)
        
        rm(clim_indexes2, db, gpsv, lgpm, lgpv)
        return(wrapClimInd)
      })
      wrapClimInd <- do.call(rbind, wrapClimInd)
      return(wrapClimInd)
  ...
  wrapFutClimInd_LGP <- do.call(rbind, wrapFutClimInd_LGP)
  ###
  
  wrapFutClimInd <- rbind(wrapFutClimInd, wrapFutClimInd_LGP)
  ...
  if(!file.exists(paste(outDir, '/', gsub(pattern=' ', replacement='_', countyList$County[g]), '_firstSeason_futureTrend_2021_2045_LGP.pdf', sep=''))){
    ggsave(filename=paste(outDir, '/', gsub(pattern=' ', replacement='_', countyList$County[g]), '_firstSeason_futureTrend_2021_2045_LGP.pdf', sep=''), plot=gg, width=8, height=9, units='in')
  }
  ...
  gg <- gg + facet_grid(Index ~ RCP, scales='free_y') + theme_bw()
  gg <- gg + theme(axis.text.x = element_text(size=7))
  if(!file.exists(paste(outDir, '/', gsub(pattern=' ', replacement='_', countyList$County[g]), '_secondSeason_futureTrend_2021_2045_LGP.pdf', sep=''))){
    ggsave(filename=paste(outDir, '/', gsub(pattern=' ', replacement='_', countyList$County[g]), '_secondSeason_futureTrend_2021_2045_LGP.pdf', sep=''), plot=gg, width=8, height=9, units='in')
  }
  ...
  gg <- gg + facet_grid(Index ~ RCP, scales='free_y') + theme_bw()
  gg <- gg + theme(axis.text.x = element_text(size=7))
  if(!file.exists(paste(outDir, '/', gsub(pattern=' ', replacement='_', countyList$County[g]), '_firstSeason_futureTrend_2041_2065_LGP.pdf', sep=''))){
    ggsave(filename=paste(outDir, '/', gsub(pattern=' ', replacement='_', countyList$County[g]), '_firstSeason_futureTrend_2041_2065_LGP.pdf', sep=''), plot=gg, width=8, height=9, units='in')
  }
  ...
  gg <- gg + facet_grid(Index ~ RCP, scales='free_y') + theme_bw()
  gg <- gg + theme(axis.text.x = element_text(size=7))
  if(!file.exists(paste(outDir, '/', gsub(pattern=' ', replacement='_', countyList$County[g]), '_secondSeason_futureTrend_2041_2065_LGP.pdf', sep=''))){
    ggsave(filename=paste(outDir, '/', gsub(pattern=' ', replacement='_', countyList$County[g]), '_secondSeason_futureTrend_2041_2065_LGP.pdf', sep=''), plot=gg, width=8, height=9, units='in')
  }
...
wrapFutClimInd_median <- lapply(1:nrow(countyList), function(g){
  ...
  wrapFutClimInd_median <- lapply(1:length(seasonList), function(h){
    ...
    wrapFutClimInd_median <- lapply(1:length(periodList), function(i){
      ...
      wrapClimInd_median <- as.data.frame(dplyr::summarise(group_by(wrapClimInd, Index, RCP, Years), median(Average, na.rm=TRUE)))
  ...
  if(!file.exists(paste(outDir, '/', gsub(pattern=' ', replacement='_', countyList$County[[g]]), '_mergeFutureTrend_2021_2045_LGP.pdf', sep=''))){
    ggsave(filename=paste(outDir, '/', gsub(pattern=' ', replacement='_', countyList$County[[g]]), '_mergeFutureTrend_2021_2045_LGP.pdf', sep=''), plot=gg, width=8, height=9, units='in')
  }
  ...
  gg <- gg + theme(axis.text.x = element_text(size=10),
                   ...
                   legend.text = element_text(size=14),
                   legend.title = element_text(face="bold",size=15))
  if(!file.exists(paste(outDir, '/', gsub(pattern=' ', replacement='_', countyList$County[[g]]), '_mergeFutureTrend_2041_2065_LGP.pdf', sep=''))){
    ggsave(filename=paste(outDir, '/', gsub(pattern=' ', replacement='_', countyList$County[[g]]), '_mergeFutureTrend_2041_2065_LGP.pdf', sep=''), plot=gg, width=8, height=9, units='in')
  }
...
trendFutAll <- lapply(1:nrow(countyList), function(i){
  ...
  if(!file.exists(paste(outDir, '/', gsub(pattern=' ', replacement='_', countyList$County[[i]]), '_significanceTrend_2021_2045_LGP.pdf', sep=''))){
    ggsave(filename=paste(outDir, '/', gsub(pattern=' ', replacement='_', countyList$County[[i]]), '_significanceTrend_2021_2045_LGP.pdf', sep=''), plot=gg, width=8, height=9, units='in')
  }
  ...
  gg <- gg + theme_bw() + geom_hline(yintercept=0.05)
  gg <- gg + ylab('p-value Mann-Kendall test') + scale_y_continuous(limits=c(0, 1))
  if(!file.exists(paste(outDir, '/', gsub(pattern=' ', replacement='_', countyList$County[[i]]), '_significanceTrend_2041_2065_LGP.pdf', sep=''))){
    ggsave(filename=paste(outDir, '/', gsub(pattern=' ', replacement='_', countyList$County[[i]]), '_significanceTrend_2041_2065_LGP.pdf', sep=''), plot=gg, width=8, height=9, units='in')
  }
...
if(!file.exists(paste(outDir, '/', gsub(pattern=' ', replacement='_', countyList$County[1]), '_allFutureTrend_2021_2065_LGP.pdf', sep=''))){
  ggsave(filename=paste(outDir, '/', gsub(pattern=' ', replacement='_', countyList$County[1]), '_allFutureTrend_2021_2065_LGP.pdf', sep=''), plot=gg, width=10, height=12, units='in')
}
...
lapply(1:length(indList), function(i){
  ...
  outDir <- paste('/mnt/workspace_cluster_8/Kenya_KACCAL/results/graphics/future_trends/', gsub(pattern=' ', replacement='_', countyList$County[1]), '/individual_plots_LGP', sep='')
  if(!dir.exists(outDir)){
    dir.create(outDir, recursive=TRUE)
  }
  if(!file.exists(paste(outDir, '/', gsub(pattern=' ', replacement='_', countyList$County[1]), '_', indList[i], '_allRCP_2021_2065_LGP.pdf', sep=''))){
    ggsave(filename=paste(outDir, '/', gsub(pattern=' ', replacement='_', countyList$County[1]), '_', indList[i], '_allRCP_2021_2065_LGP.pdf', sep=''), plot=gg, width=12, height=12, units='in')
  }
})
...
lapply(1:length(indList), function(i){
  ...
  lapply(1:length(rcpList), function(j){
    ...
    if(!file.exists(paste(outDir, '/', gsub(pattern=' ', replacement='_', countyList$County[1]), '_', indList[i], '_', rcpList[j], '_2021_2065_LGP.pdf', sep=''))){
      ggsave(filename=paste(outDir, '/', gsub(pattern=' ', replacement='_', countyList$County[1]), '_', indList[i], '_', rcpList[j], '_2021_2065_LGP.pdf', sep=''), plot=gg, width=12, height=10, units='in')
    }
...
wrapFutClimInd2 <- as.data.frame(dplyr::summarise(group_by(wrapFutClimInd2, Index, GCM, RCP, County, Season, Years), median(Average, na.rm=TRUE)))
...
scenarioTrendsClustering <- function(county){
  ...
  # Load growing seasons estimations
  load(paste('/mnt/workspace_cluster_8/Kenya_KACCAL/results/climatic_indices/historical/growing_seasons/', gsub(pattern=' ', replacement='_', county), '_growingSeasons.RData', sep=''))
  db <- clim_indexes2[[1]]
  db <- db[,c('cellID', 'gSeason', paste(1981:2005))]
  # GPSV: Stability in start of season
  gpsv <- data.frame(Average=as.numeric(apply(X = db[db$gSeason==1, 3:ncol(db)], MARGIN = 2, FUN = sd, na.rm=TRUE)), Index='GPSV', Years=1981:2005, Season='First')
  # LGPM: Length of growing season
  db <- clim_indexes2[[2]]
  db <- db[,c('cellID', 'gSeason', paste(1981:2005))]
  lgpm <- data.frame(Average=as.numeric(apply(X = db[db$gSeason==1, 3:ncol(db)], MARGIN = 2, FUN = median, na.rm=TRUE)), Index='LGPM', Years=1981:2005, Season='First')
  # LGPV: Variability of LGP
  lgpv <- data.frame(Average=as.numeric(apply(X = db[db$gSeason==1, 3:ncol(db)], MARGIN = 2, FUN = sd, na.rm=TRUE)), Index='LGPV', Years=1981:2005, Season='First')
  allWrap <- rbind(gpsv, lgpm, lgpv); rm(gpsv, lgpm, lgpv)
  
  ###
  indList2 <- as.character(unique(allWrap$Index))
  first_seasonWrap2 <- lapply(1:length(indList2), function(m){
    timeSer <- allWrap[allWrap$Index==indList2[m],]
    grep2 <- Vectorize(grep, vectorize.args='pattern')
    timeSer <- ts(timeSer$Average[grep2(pattern=1981:2005, timeSer$Years, fixed=TRUE)], start=min(timeSer$Years), end=max(timeSer$Years), frequency=1)
    slope <- sens.slope(timeSer); slope <- slope$b.sen
    df <- data.frame(slope=slope, Index=paste(indList2[m], '-S1', sep=''))
    return(df)
  }); rm(allWrap)
  first_seasonWrap2 <- do.call(rbind, first_seasonWrap2)
  first_seasonWrap <- rbind(first_seasonWrap, first_seasonWrap2)
  first_seasonWrap$Index <- as.character(first_seasonWrap$Index)
  first_seasonWrap <- first_seasonWrap[order(first_seasonWrap$Index),]; rownames(first_seasonWrap) <- 1:nrow(first_seasonWrap)
  ###
  ...
  db <- db[,c('cellID', 'gSeason', paste(1981:2005))]
  # GPSV: Stability in start of season
  gpsv <- data.frame(Average=as.numeric(apply(X = db[db$gSeason==2, 3:ncol(db)], MARGIN = 2, FUN = sd, na.rm=TRUE)), Index='GPSV', Years=1981:2005, Season='Second')
  # LGPM: Length of growing season
  db <- clim_indexes2[[2]]
  db <- db[,c('cellID', 'gSeason', paste(1981:2005))]
  lgpm <- data.frame(Average=as.numeric(apply(X = db[db$gSeason==2, 3:ncol(db)], MARGIN = 2, FUN = median, na.rm=TRUE)), Index='LGPM', Years=1981:2005, Season='Second')
  # LGPV: Variability of LGP
  lgpv <- data.frame(Average=as.numeric(apply(X = db[db$gSeason==2, 3:ncol(db)], MARGIN = 2, FUN = sd, na.rm=TRUE)), Index='LGPV', Years=1981:2005, Season='Second')
  allWrap <- rbind(gpsv, lgpm, lgpv); rm(gpsv, lgpm, lgpv)
  
  ###
  indList2 <- as.character(unique(allWrap$Index))
  second_seasonWrap2 <- lapply(1:length(indList2), function(m){
    timeSer <- allWrap[allWrap$Index==indList2[m],]
    grep2 <- Vectorize(grep, vectorize.args='pattern')
    timeSer <- ts(timeSer$Average[grep2(pattern=1981:2005, timeSer$Years, fixed=TRUE)], start=min(timeSer$Years), end=max(timeSer$Years), frequency=1)
    slope <- sens.slope(timeSer); slope <- slope$b.sen
    df <- data.frame(slope=slope, Index=paste(indList2[m], '-S2', sep=''))
    return(df)
  }); rm(allWrap)
  second_seasonWrap2 <- do.call(rbind, second_seasonWrap2)
  second_seasonWrap <- rbind(second_seasonWrap, second_seasonWrap2)
  second_seasonWrap$Index <- as.character(second_seasonWrap$Index)
  second_seasonWrap <- second_seasonWrap[order(second_seasonWrap$Index),]; rownames(second_seasonWrap) <- 1:nrow(second_seasonWrap)
  rm(clim_indexes2)
  ###
    ...
    df <- lapply(1:length(gcmList), function(o){
      df <- lapply(1:length(rcpList), function(p){
        df <- lapply(1:length(seasonList), function(r){
          ...
          timeSer$Average[which(is.na(timeSer$Average))] <- mean(timeSer$Average, na.rm=TRUE)
          grep2 <- Vectorize(grep, vectorize.args='pattern')
  ...
  diag(corMat) <- NA
  outDir <- paste('/mnt/workspace_cluster_8/Kenya_KACCAL/results/graphics/cluster_analysis/combined/trends_allYears_LGP/', gsub(pattern=' ', replacement='_', county), sep='')
  if(!dir.exists(outDir)){dir.create(outDir, recursive=TRUE)}
  if(!file.exists(paste(outDir, '/', gsub(pattern=' ', replacement='_', county), '_correlationMatrix.pdf', sep=''))){
    pdf(paste(outDir, '/', gsub(pattern=' ', replacement='_', county), '_correlationMatrix.pdf', sep=''), height=8, width=8)
    heatmap.2(corMat, col=my_palette, density.info="none", trace="none", dendrogram="column", key.title='', key.xlab='Spearman correlation', margins=c(9,9))
  ...
  rm(aux)
  names(df_cluster)[1] <- 'Scenario'
  df_cluster$Index <- factor(x=df_cluster$Index, levels=c('TMEAN', 'GDD_1', 'GDD_2', 'ND_t35', 'TOTRAIN', 'CDD', 'P5D', 'P_95', 'NDWS', 'GPSV', 'LGPM', 'LGPV'))
